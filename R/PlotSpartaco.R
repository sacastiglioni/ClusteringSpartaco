...
#' @param geneList a vector of length `nrow(x$x)`, containing the gene names. If `NULL`, the function takes `gene.names = row.names(x$x)`.
#'
#'
#'
#' @return The requested plot is displayed. In addition, if assigned to an object, it will return the `ggplot` object.
#'
plot.spartaco <- function(x, type = 1, gene.name = readline("gene name: "), k = NULL, r = 1:ncol(x$mu), manual.palette = NULL, display.all.spots = T, remove.outliers = F, coef = 1, range = NULL, geneList = NULL, ...){
  if(class(x) != "spartaco") stop("the input file is not a spartaco object")
  if(length(type) > 1) type <- 1
  if(is.null(geneList)) geneList <- row.names(x$x)
  else row.names(x$x) <- geneList
  K <- nrow(x$mu)
  R <- ncol(x$mu)
  k.lab <- 1:K
  r.lab <- 1:R
    ...
    if(!display.all.spots){
      if(length(k) <= 1){
        if(length(k) == 0){
          ...
          if(is.null(range)){
            min <- round(min(x.bar),2)
            max <- round(max(x.bar),2)
          }
          else{
            min <- range[1]
            max <- range[2]
